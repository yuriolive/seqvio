...
- **A pen parked near the top-left corner while something else is drawing is a coordinate-space bug, never a timing one.** `getPointAtLength` returns the path's own local space, so any drawable rendering inside a transformed `<g>` (`DrawIcon` scales a 24x24 viewBox) must map through `getCTM()` before reporting to the draw registry.
- **Probe a frame mid-icon-draw before delivering.** Icon draws are brief and single-pen scheduling delays their effective start past the authored `start`, so scan a ~120 frame window rather than one computed frame. Field-notes item 10 has the command.
...
- **Paint order is JSX order, not draw order.** Every drawable is its own absolutely positioned `<svg>`, so an arrow authored after a label covers it. Author labels after the connectors they sit among, and better, place them outside the corridor the arrows occupy. See field-notes item 11.
- **`DrawText` has no rotation — budget width for every label.** Estimate `0.5 * fontSize * characters` px and keep it clear of axes, threshold lines, and bars; there is no vertical axis label. See field-notes item 12.
